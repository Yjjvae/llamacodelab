...
暴力检索的 p95 为 1,617 µs。`ef_search=256` 在达到更高召回的同时仍显著快于暴力检索，因此作为评测
...
hnswlib v0.8.0 的手写 SSE 预取在短邻接表上会触发越界读取；适配器仅在包含该依赖的编译单元定义
`NO_MANUAL_VECTORIZATION`。这牺牲部分延迟以通过 ASan/UBSan，且不会影响其他模块的 SIMD 配置。
